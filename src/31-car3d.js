...
function makeWheel(THREE, build, spec, side, front){
  ...
  /* spin 子群組只放「會跟著車輪自轉」的件（胎、輪輞、輪輻面、輪唇）。
     懸吊與卡鉗留在外層 —— 駕駛模式的自轉只轉 spin，彈簧不會跟著打轉。 */
  const spin=new THREE.Group();g.add(spin);g.userData.spin=spin;
  mesh(THREE,spin,tireProfileGeometry(THREE,spec),mat(THREE,0x111312,{roughness:.88,metalness:0}));
  addTireDetails(THREE,spin,spec);
  ...
  mesh(THREE,spin,new THREE.CylinderGeometry(spec.rimR*.945,spec.rimR*.945,spec.rimW*.96,64,1,true),
       barrelMat,[0,0,0],[Math.PI/2,0,0]);

  spin.add(wheelFaceGroup(THREE,preset,spec,hub,rimMat,lipMat,
  ...
  outer.position.z=side*spec.rimW/2; spin.add(outer);
  const inner=new THREE.Mesh(new THREE.TorusGeometry(spec.rimR*.97,lipW*.72,8,48),rimMat);
  inner.position.z=-side*spec.rimW/2; spin.add(inner);
  return g;
}
...
function addLightingParts(THREE, body, spec){
  ...
  /* 命名讓程序化燈具也能被燈光開關（car3DLampRoles）認出來 */
  head.name='car3d-head';tail.name='car3d-tail';turn.name='car3d-turn';
  if(spec.eclipse){
    [-1,1].forEach(side=>{
...
/* 把俯仰套用到車身。
   normalizeImportedCar() 會對「長度在 Z 軸」的網格設 rotation.y = -π/2，
...
function buildProceduralCarModel(THREE, spec, build){
  ...
  body.position.y=stance.dy;applyBodyTilt(THREE,body,stance.tilt);
...
async function buildImportedCarModel(THREE, GLTFLoader, MeshoptDecoder, spec, build){
  ...
  applyBodyTilt(THREE,model,stance.tilt);
...
async function createScene(root, config, THREE, OrbitControls, GLTFLoader, MeshoptDecoder){
  ...
  const hemi=new THREE.HemisphereLight(dark?0xb7d6dd:0xe8f3ef,dark?0x172018:0x6d756d,2.2);scene.add(hemi);
  const key=new THREE.DirectionalLight(0xffffff,4.0);key.position.set(-4.5,7,5);key.castShadow=true;
  key.shadow.mapSize.set(1024,1024);key.shadow.camera.left=-6;key.shadow.camera.right=6;key.shadow.camera.top=5;key.shadow.camera.bottom=-2;scene.add(key);scene.add(key.target);
  ...
  if(controls){
    controls.enabled=!(CAR3D_DRIVE.on&&CAR3D_DRIVE.follow);
    let lastT=0;
  ...
  CAR3D_READY.then(({THREE})=>CAR3D_INSTANCES.forEach(x=>{
    ...
    x.car.traverse(o=>{
      if(o.userData.vehicleBody){
        ...
        applyBodyTilt(THREE,o,stance.tilt);
      }
...
/* ==========================================================================
   互動預覽 · 模組一：燈光開關
   --------------------------------------------------------------------------
   狀態放 build.lights（跟著車輛存進 DB）：
     {head:0|1|2|3, brake:bool, turn:'none'|'hazard', night:bool}
     head：0=關 / 1=小燈 / 2=近燈 / 3=遠燈
   材質辨識靠名稱（三套車體命名各不相同，尾碼 .0xx 先剝掉再比對）。
   E36 的 orangeglass 與 M3 的 SideLamp_lens 是獨立方向燈材質；
   Eclipse 的方向燈併在 Headlight_Bucket 裡，所以閃爍時整個燈碗一起呼吸 ——
   這是來源網格材質切分的極限，不再硬切幾何（會重蹈破面覆轍）。
   左右分開閃做不到（同一顆材質蓋左右兩邊），所以方向燈做成雙黃警示燈。
   ========================================================================== */
function car3DLightState(build){
  const l=(build&&build.lights)||{};
  return {head:Math.max(0,Math.min(3,+l.head||0)), brake:!!l.brake,
          turn:l.turn==='hazard'?'hazard':'none', night:!!l.night};
}
/* 一顆材質可以身兼多職（Eclipse 燈碗＝大燈＋方向燈） */
function car3DLampRoles(rawName){
  const n=String(rawName||'').toLowerCase().replace(/\.\d+$/,'');
  if(/^clearglass/.test(n)||n==='car3d-head'||n==='headlamp_lens'||n==='headlight_inner') return {head:1};
  if(n==='headlight_bucket') return {head:1,turn:1};
  if(/^orangeglass/.test(n)||n==='sidelamp_lens'||n==='car3d-turn') return {turn:1};
  if(/^redglass/.test(n)||n==='taillight_red'||n==='rearlamp_lens'||n==='car3d-tail') return {tail:1};
  return null;
}
function collectCar3DLamps(model){
  const found=[],seen=new Set();
  model.traverse(o=>{
    if(!o.isMesh||!o.material) return;
    (Array.isArray(o.material)?o.material:[o.material]).forEach(m=>{
      if(!m||seen.has(m)||!m.emissive) return;
      const roles=car3DLampRoles(m.name); if(!roles) return;
      seen.add(m);
      found.push({m,roles,base:{hex:m.emissive.getHex(),i:m.emissiveIntensity??1}});
    });
  });
  return found;
}
let CAR3D_TURN_PHASE=true;
function renderCar3DOnce(x){ if(!x.dead) x.renderer.render(x.scene,x.camera); }
function applyCar3DLamps(x){
  ...
  x.lamps.forEach(({m,roles,base})=>{
    let hex=null,i=0;
    if(roles.tail&&tailI>0){hex=0xff1a1a;i=tailI;}
    if(roles.head&&headI>0){hex=0xfff4e0;i=headI;}
    if(roles.turn&&turnOn){
      if(CAR3D_TURN_PHASE){hex=0xff9500;i=2.0;}
      else if(hex===null){hex=0xff9500;i=0;}       // 暗相：獨立方向燈全暗，共用燈碗回到大燈檔
    }
    if(hex===null){m.emissive.setHex(base.hex);m.emissiveIntensity=base.i;}
    else{m.emissive.setHex(hex);m.emissiveIntensity=i;}
  });
  if(x.beams) x.beams.forEach(b=>{b.visible=L.head>=2;b.intensity=L.head>=3?b.userData.hi:b.userData.lo;});
  else if(L.head>=2) ensureCar3DBeams(x);
  if(!x.loop) renderCar3DOnce(x);
}
/* 近燈/遠燈的實體光束（掛在車身群組上，駕駛模式會跟著車走） */
function ensureCar3DBeams(x){
  if(x.beams||!x.car||x.dead) return;
  const {THREE,spec}=x;
  x.beams=[-1,1].map(side=>{
    const s=new THREE.SpotLight(0xffeecf,0,34,.42,.6,1.5);
    s.position.set(-spec.length/2+.06,.66,side*spec.width*.29);
    s.target.position.set(-11,-.5,side*spec.width*.18);
    s.castShadow=false;s.userData.lo=60;s.userData.hi=170;
    x.car.add(s);x.car.add(s.target);
    return s;
  });
  applyCar3DLamps(x);
}
function applyCar3DNight(x,night){
  if(x.dead||!x.env) return;
  const e=x.env;
  e.hemi.intensity=night?.25:2.2;      // 規格：白天環境光太強，燈亮不亮看不出來
  e.key.intensity=night?.5:4.0;
  e.rim.intensity=night?.3:2.0;
  e.warm.intensity=night?.08:.7;
  /* 夜間鋪一塊真的地面：ShadowMaterial 只收影子，SpotLight 的光束要有面可打 */
  if(night&&!e.nightGround){
    const g=new x.THREE.Mesh(
      new x.THREE.PlaneGeometry(60,60),
      new x.THREE.MeshStandardMaterial({color:0x161c22,roughness:.94,metalness:0}));
...
function updateCar3DLights(build){
  const L=car3DLightState(build);
  CAR3D_INSTANCES.forEach(x=>{
    if(x.dead) return;
    const prevNight=!!(x.lightState&&x.lightState.night);
    x.lightState=L;
    if(prevNight!==L.night||x.nightApplied!==L.night){x.nightApplied=L.night;applyCar3DNight(x,L.night);}
    applyCar3DLamps(x);
  });
}
/* 方向燈閃爍：規格指定 setInterval 每 400 ms 切換 */
setInterval(()=>{
  CAR3D_TURN_PHASE=!CAR3D_TURN_PHASE;
  CAR3D_INSTANCES.forEach(x=>{
    if(!x.dead&&x.lightState&&x.lightState.turn==='hazard') applyCar3DLamps(x);
  });
},400);
